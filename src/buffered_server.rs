...
fn id_to_range<'a>(range: &StringRegion, args: &Vec<String>, zoom: u64, path: u64, param: &Param, path_string: String) -> (ArgMatches, StringRegion) {
    ...
    let b: Vec<String> = if (y >> (max_zoom-zoom)) <= 2 { // i.e. 2**22s
        vec!["-A".to_string(), "-Y".to_string(), (max_y >> (max_zoom-zoom)).to_string(), "-y".to_string(), (y >> (max_zoom-zoom)).to_string(), "-n".to_string(), "-I".to_string(), "-o".to_string(), path_string, "-X".to_string(), ((max_y >> (max_zoom-zoom)) / 2).to_string(), "-x".to_string(), x.to_string(), "-l".to_string(), "-e".to_string()]
...
pub async fn server(matches: ArgMatches, range: StringRegion, prefetch_range: StringRegion, args: Vec<String>, buffer:ChromosomeBuffer,threads: u16) -> std::io::Result<()> {
    ...
    HttpServer::new(move|| {
        //let list = list.clone();
        actix_web::App::new().data(()).app_data(web::Data::new(RwLock::new(list.clone()))).app_data(counter.clone()).app_data(buffer.clone()).route("/", web::get().to(get_index))
