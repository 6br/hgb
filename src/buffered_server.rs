use crate::server::{Image, Size, DZI};
use actix_cors::Cors;
...
fn id_to_range(
    ...
    let b: Vec<String> = if (y >> (max_zoom - zoom)) <= 2 {
        ...
        vec![
            ...
            path_string,
            "-X".to_string(),
            (adjusted_large_y / 2).to_string(),
            "-x".to_string(),
            x.to_string(),
            "-l".to_string(),
            "-e".to_string(),
        ]
    } else if criteria << (max_zoom - zoom) <= 10000 && (y >> (max_zoom - zoom)) >= 8 {
        ...
        vec![
            ...
            adjusted_y.to_string(),
            "-X".to_string(),
            (scalex_default >> (max_zoom - zoom)).to_string(),
            "-o".to_string(),
            path_string,
            "-x".to_string(),
            x.to_string(),
...
pub async fn server<T: 'static + ChromosomeBufferTrait + Send + Sync>(
    ...
    let zoom_range = matches
        .value_of("zoom-range")
        .and_then(|a| a.parse::<u32>().ok())
        .unwrap_or(if matches.is_present("whole-chromosome") {
            6u32
        } else {
            8u32
        });
    ...
    eprintln!(
        "Total len: {}, partial len: {}, x_width: {}({}), x: {}",
        all, diff, x_width, x_width as i64, x
    );
    ...
    HttpServer::new(move || {
        ...
        actix_web::App::new()
            ...
            .app_data(web::Data::new(list_btree)) /*.app_data(web::Data::new(RwLock::new(list.clone()))).*/
