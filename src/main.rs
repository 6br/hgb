...
use ghi::range::{InvertedRecordEntire, Set};
...
//            .multiple(true)
    ...
    let threads = matches.value_of("threads").and_then(|t| t.parse::<u16>().ok()).unwrap();
...
fn build(matches: &ArgMatches, threads: u16) -> () {
    ...
    for bed_path in bed_files {
        ...
        let set: Set<InvertedRecordBuilder> = Set::<InvertedRecordBuilder>::new(reader, 1 as u64, &mut header).unwrap();
    ...
    let mut writer = GhbWriter::build()
        .write_header(false)
        .from_path(output_path, dummy_header).unwrap();
    let index = entire.write_binary(&mut writer).unwrap();
    writer.flush().unwrap();
    let output_index_path = format!("{}.ghi", output_path);
    let mut index_writer = GhiWriter::build().write_header(true).from_path(output_index_path, header).unwrap();
...
fn serve(matches: &ArgMatches, threads: u16) -> () {
